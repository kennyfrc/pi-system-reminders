...
interface ReminderSection {
  ...
  text: string;
  priority: number;
  /** Durable sections carry the durable="true" marker on their <reminder> tag. */
  durable?: boolean;
}
...
export function gateMatches(gate: Gate, messages: AgentMessage[]): boolean {
  ...
  switch (gate) {
    ...
    case "session:start":
      // The session's first user prompt: a plain-user tail AND no assistant or
      // tool traffic anywhere in the request history. After the first model
      // reply this can never be true again — the once-per-session contract.
      return (
        roleOf(tail) === "user" &&
        !messages.some((m) => {
          const role = roleOf(m);
          return role === "assistant" || role === "toolResult" || role === "tool";
        })
      );
  }
}
...
function renderBlock(kept: ReminderSection[]): string {
  const sections = kept
    .map((r) =>
      r.durable
        ? `<reminder type="${r.label}" durable="true">\n${r.text}\n</reminder>`
        : `<reminder type="${r.label}">\n${r.text}\n</reminder>`,
    )
    .join("\n");
  ...
  // (durable sections additionally carry durable="true"). The
  // <system_reminder> wrapper itself carries no attributes. Matches the
  // singular tag the static-compactor strips (open + close on their own lines).
  return `<system_reminder>\n${sections}\n</system_reminder>`;
}

/**
 * Evaluate the durable (session:start) reminders against the captured context
 * files and render their persisted block: the SAME `<system_reminder>`
 * envelope the transient channel uses, each section marked
 * `durable="true"`. Sent ONCE via pi.sendMessage at the session's first user
 * prompt, persisted in the session transcript, replayed in every later
 * request and on resume. Returns null when no durable content applies.
 */
export function renderDurableBlock(
  reminders: Reminder[],
  contextFiles: ReadonlyArray<{ path: string; content: string }> | undefined,
): { text: string; reminderIds: string[] } | null {
  const fired: FiredReminder[] = [];
  for (const r of reminders) {
    if (r.lifetime !== "durable") continue;
    let text: string | null = null;
    try {
      text = r.content({ messages: [], contextFiles });
    } catch {
      continue; // a throwing provider is skipped, never breaks the session
    }
    if (text == null || text.trim() === "") continue;
    fired.push({ ...sectionOf(r, text, true), id: r.id });
  }
  if (fired.length === 0) return null;
  return { text: renderBlock(fired), reminderIds: fired.map((f) => f.id) };
}

/** Build a render section from a reminder and its evaluated text. */
function sectionOf(r: Reminder, text: string, durable: boolean): ReminderSection {
  return { label: r.label, text, priority: r.priority ?? DEFAULT_PRIORITY, durable };
}
 ...
 *
 * Transient reminders ONLY. Durable (session:start) reminders never ride this
 * path: they are persisted as a session message once at the first user prompt
 * (see renderDurableBlock + the before_agent_start sender in index.ts) and
 * then replay from the transcript in every request, so re-deriving them here
...
export function applyReminders(
  ...
  const transient = reminders.filter((r) => r.lifetime !== "durable");
  ...
  for (const r of transient) {
    ...
    if (text == null || text.trim() === "") continue;
    fired.push({ ...sectionOf(r, text, false), id: r.id });
  }
